  ...
  **/
  ...
  $.SrBuj = SrBuj = {
    ...
    , success: function (e, data, status, xhr, user_options) {
      if (xhr && $.SrBuj.wrongHeader.test(xhr.getResponseHeader('Content-Type'))) return
    ...
    , Util: {
      ...
      , notify: function (user_options) {
        ...
        gsMsg.className = this.defaults.matchingClasses.test(options.type) ? ['alert', '-', options.type].join('') : (options.type || 'alert-info')
